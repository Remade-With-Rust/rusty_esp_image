...
| **I1** (J1) ◐ host half 2026-09-01 | Track A `IdfCamera` on XIAO S3 Sense (OV2640, JPEG mode). **Written:** `idf::IdfCamera` over esp32-camera, `CameraPins` for the XIAO S3 Sense and the ESP32-CAM; linked by `rusty_esp_video/firmware/xiao-s3-sense-idf-mjpeg` (first ESP-IDF build pending) | 320×240 JPEG frames counted on serial at a recorded FPS for 10 minutes with zero pool exhaustion; every frame passes `Frame::packed` (starts `FF D8`) — **needs the board** |
